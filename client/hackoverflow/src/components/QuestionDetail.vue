...
export default {
  ...
  methods: {
    ...
    postAnswer() {
      ...
      axios.post(
        `http://localhost:3000/questions/answers/${this.id}`,
        newAnswer,
    ...
    voteAnswer(stat,idanswer) {
      ...
      if(stat == 'up') {
        ...
        axios.put(
          `http://localhost:3000/questions/answers/downvotes/${this.id}`,
          vote,
          {headers: {'token': window.localStorage.getItem('token')}})
          .then(function(res) {
            if(res.data.success) {
    ...
    voteQuestion(stat) {
      ...
      if(stat == 'up') {
        axios.put(
          `http://localhost:3000/questions/upvotes/${this.id}`,
          {
            user: window.localStorage.getItem('id')
          },
          {headers: {'token': window.localStorage.getItem('token')}})
        ...
        axios.put(
          `http://localhost:3000/questions/downvotes/${this.id}`,
          {
            user: window.localStorage.getItem('id')
          },
          {headers: {'token': window.localStorage.getItem('token')}})
